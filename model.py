...
class OpenUnmix_multitask(nn.Module):
    def __init__(
        self,
        n_fft=4096,
        n_hop=1024,
        ...
        super(OpenUnmix_multitask, self).__init__()
...
class OpenUnmix_multiinp(nn.Module):
    def __init__(
        ...
        super(OpenUnmix_multiinp, self).__init__()
        ...
        self.fc1 = Linear(
            (self.nb_bins+1)*nb_channels, hidden_size,
            bias=False
        )
    ...
    def forward(self, x, y_inst):
        ...
        #y_inst = x.sum(-1)/ self.nb_output_bins
        #y_inst[y_inst>0]=1
        #y_inst = y_inst.unsqueeze(-1)
        y_inst = y_inst.permute(2,0,1).unsqueeze(3)
        l = (x.shape[0]-y_inst.shape[0])
        y_inst[y_inst>0]=1
        y_inst = F.pad(y_inst,(0,0,0,0,0,0,0,l),'constant',0)

        x = torch.cat((x, y_inst), -1)
        # to (nb_frames*nb_samples, nb_channels*nb_bins)
        # and encode to (nb_frames*nb_samples, hidden_size)
        x = self.fc1(x.reshape(-1, nb_channels*(self.nb_bins+1)))
        x = self.bn1(x)
        x = x.reshape(nb_frames, nb_samples, self.hidden_size)
        x = torch.tanh(x)
        lstm_out = self.lstm(x)
