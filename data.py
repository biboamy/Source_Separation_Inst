from utils import load_audio, load_info, compute_activation_confidence
...
class FixedSourcesTrackFolderDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        target_audio = self.source_augmentations(target_audio) # modify
        ####
        #target_audio = torch.sum(target_audio,0,True)/2  
        ####
        ...
        for source in self.interferer_files:
            ...
            if self.random_track_mix:
                ...
                if self.random_chunks:
                    start = random.uniform(0, min_duration - self.seq_duration)
            audio = load_audio(
                track_path / source, start=0, dur=self.seq_duration
            )
            ####
            #audio = torch.sum(audio,0,True)/2  
            ####
            audio = self.source_augmentations(audio) #modify
            audio_sources.append(audio)
        ...
        if self.split == 'train' and self.seq_duration:
            for k, source in enumerate(self.mus.setup['sources']):
                ...
                if source == self.target:
                    target_ind = k
                    inst_track = track.sources[source].audio
